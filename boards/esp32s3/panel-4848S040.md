...
The display touch controller is connected using the [i2c bus](/dev/i2c.md).
The ESP32 pins in used are specified in the device configuration.

## SPI bus

The display and the SD card are using the [SPI bus](/dev/spi.md).
The ESP32 pins in used are specified in the device configuration.


## Display

The IPS LCD display is supporting a resolution of 480 * 480 pixels and 16-bit colors and is
based on the ST7701 chip.

It is connected to the processor using the ESP32-S3 specific 16-bit data bus using DMA to
transfer for pixel data and the SPI bus in 9-bit mode for commands and configuration.

The SPI bus is using the 9-bit data transfer can only be used for sending information. As the
9-bit mode is not supported by the Arduino environment the spi bus configuration must be given
