...
- call <http://plug/$board/bl0937/0?cycletime=5000> to create a more precise pulse monitoring 5 sec cycles can be used.
- call <http://plug/$board/bl0937/0?mode=voltage> 
- Switch on and wait some cycles (30secs).
- call <http://plug/$board/bl0937/0?poweradjust=100> to adjust the powerfactor
- call <http://plug/$board/bl0937/0?voltageadjust=230> to adjust the voltagefactor
- calculate the current in mA using the formula current = watt/voltage*1000
- call <http://plug/$board/bl0937/0> and see the powerfactor and voltagefactor
- call <http://plug/$board/bl0937/0?mode=current> 
- Wait some cycles (30secs).
- call <http://plug/$board/bl0937/0?currentadjust=435>
- call <http://plug/$board/bl0937/0> and see the currentfactor
